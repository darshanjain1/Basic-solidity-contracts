...
const { safeSub } = require('safemath')
    ...
    : describe("Random IPFS NFT Unit Tests", function () {
        ...
        describe("constructor", () => {
            it("sets starting values correctly", async function () {
                ...
                assert(dogTokenUriZero.includes(process.env.PINATA_IPFS_DOMAIN))
        ...
        describe("mintNft", () => {
            it("fails if payment isn't sent with the request", async function () {
                await expect(randomIpfsNft.mintNft()).to.be.revertedWith(
                    "RandomIpfsNft__NotEnoughEtherSent"
                )
            })
            it("reverts if payment amount is less than the mint fee", async function () {
                ...
                await expect(
                    randomIpfsNft.mintNft({
                        value: safeSub(fee, ethers.parseEther("0.001")),
                    })
                ).to.be.revertedWith("RandomIpfsNft__NotEnoughEtherSent")
            })
            it("emits an event and kicks off a random word request", async function () {
                const fee = await randomIpfsNft.getMintFee()
                await expect(randomIpfsNft.mintNft({ value: fee.toString() })).emit(
                    randomIpfsNft,
                    "NftRequested"
                )
            })
                ...
                await new Promise(async (resolve, reject) => {
                    randomIpfsNft.once("NftMinted", async () => {
                        try {
                            ...
                            assert.equal(tokenUri.toString().includes(process.env.PINATA_IPFS_DOMAIN), true)
                    ...
                    try {
                        const fee = await randomIpfsNft.getMintFee()
                        const mintNftResponse = await randomIpfsNft.mintNft({
                            value: fee.toString(),
                        })
                        const mintNftReceipt = await mintNftResponse.wait(1)
                        console.log('mintNftResponse,mintNftReceipt', mintNftResponse,mintNftReceipt)
                        await vrfCoordinatorV2Mock.fulfillRandomWords(
                            mintNftReceipt.logs[0].args[0],
